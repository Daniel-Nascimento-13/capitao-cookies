...
// ─────────────────────────────────────────────────────────────
// [ 1 ] VÍDEO — FADE CONFORME O SCROLL + PAUSA QUANDO INVISÍVEL
// QUANTO MAIS O USUÁRIO DESCE, MAIS O VÍDEO SOME.
// A 600PX DE SCROLL O VÍDEO CHEGA A OPACITY 0 E É PAUSADO
// PARA NÃO CONSUMIR CPU/GPU/BATERIA EM BACKGROUND.
// ─────────────────────────────────────────────────────────────
(function () {
  ...
  window.addEventListener("scroll", () => {
    if (!ticking) {
      requestAnimationFrame(() => {
        const op = 1 - window.scrollY / 600;
        video.style.opacity = op < 0 ? 0 : op;

        if (op <= 0 && !pausado) { video.pause(); pausado = true; }
        else if (op > 0 && pausado) { video.play(); pausado = false; }
...
// SÓ ANIMA QUANDO A SEÇÃO ESTÁ VISÍVEL (INTERSECTION OBSERVER).
...
(function () {
  ...
  function iniciar() {
    ...
    // SÓ ANIMA QUANDO O CARRINHO ESTIVER VISÍVEL NA TELA
    const io = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting && !animandoChuva) {
          animandoChuva = true;
          animar();
        } else if (!entry.isIntersecting && animandoChuva) {
          animandoChuva = false;
    ...
    io.observe(document.getElementById('carrinho'));
  }

  // RESIZE COM DEBOUNCE — EVITA DISPAROS EXCESSIVOS EM MOBILE
  let resizeTimer;
